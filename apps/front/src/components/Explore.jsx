...
export default function PublicEvents() {
  ...
  useEffect(() => {
    const fetchPublicEvents = async () => {
      ...
      try {
        ...
        // Buscar usuário atual para filtrar seus próprios eventos
  ...
  const upcomingEvents = events.filter((e) => isUpcoming(e.date_start || e.date));
  const pastEvents = events.filter((e) => !isUpcoming(e.date_start || e.date));
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "past") list = list.filter((e) => !isUpcoming(e.date_start || e.date));
    ...
    // Ordenação: eventos futuros primeiro, depois eventos passados no final
    if (sortBy === "date") {
      list.sort((a, b) => {
        const aIsUpcoming = isUpcoming(a.date_start || a.date);
        const bIsUpcoming = isUpcoming(b.date_start || b.date);
        
        // Se um é futuro e outro é passado, futuro vem primeiro
        if (aIsUpcoming && !bIsUpcoming) return -1;
        if (!aIsUpcoming && bIsUpcoming) return 1;
        
        // Ambos futuros ou ambos passados, ordenar por data
        return new Date(a.date_start || a.date) - new Date(b.date_start || b.date);
      });
    ...
    return list;
  }, [events, userId, filter, search, sortBy]);
      ...
      <div className="min-h-screen bg-[#0a0a0f] font-['Nunito',sans-serif] text-white">
        ...
        <div className="max-w-4xl mx-auto px-4 py-8 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <Stat value={events.filter(e => e.createdBy !== userId).length} label="Eventos disponíveis" Icon={Ticket} />
            <Stat value={upcomingEvents.filter(e => e.createdBy !== userId).length} label="Próximos eventos" Icon={CalendarCheck} trend="novos" />
