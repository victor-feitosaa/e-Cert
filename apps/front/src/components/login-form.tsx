...
async function onSubmit(data: LoginInput) {
  try {
    const res = await fetch("/api/auth/login", {
      ...
      credentials: "include", // garante que o cookie jwt é recebido
    })

    const json = await res.json()

    if (!res.ok) {
      // sua API retorna { error: "..." }
      setError("root", { message: json.error ?? "Erro ao fazer login" })
      return
    }

    // cookie já foi setado pela API, só redireciona
        ...
        <CardContent className="grid p-0 md:grid-cols-2">
          ...
          >
            <FieldGroup>
              ...
              {/* Erro geral da API (credenciais erradas, 500, etc.) */}
              {errors.root && (
