...
export default function CreateEvent({ onBack }) {
  ...
  const addTeamMember = () => {
    ...
    if (teamMembers.some((m) => m.email === newMemberEmail.trim().toLowerCase())) {
      setErrors((prev) => ({ ...prev, teamMemberEmail: "E-mail já adicionado" }));
      return;
    }
  ...
  // ========== SEÇÕES COM VALIDAÇÃO DE PERÍODO E CAPACIDADE ==========
  const addSectionToSubevent = () => {
    ...
    setErrors((prev) => ({ ...prev, sectionError: "", sectionCapacity: "" }));
  };
  ...
  const addMemberToSubevent = () => {
    ...
    if (!email) {
      setErrors((prev) => ({ ...prev, subeventMemberEmail: "E-mail é obrigatório" }));
      return;
    }
    if (!email.includes("@")) {
      setErrors((prev) => ({ ...prev, subeventMemberEmail: "E-mail inválido" }));
      return;
    }
    if (!job) {
    ...
    setNewSubeventMember({ email: "", job: "", fromExisting: false });
    setErrors((prev) => ({ ...prev, subeventMemberEmail: "", subeventMemberJob: "" }));
  };
    ...
    try {
      ...
      for (const subevent of subevents) {
        ...
        for (const section of subevent.sections) {
          await fetch(`/api/events/${createdEventId}/subevents/${subeventId}/sections`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            body: JSON.stringify({
        ...
        for (const member of subevent.teamMembers) {
          await fetch(`/api/events/${createdEventId}/subevents/${subeventId}/team/invite`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            body: JSON.stringify({ email: member.email, job: member.job }),
          }).catch((err) => console.error(`Erro ao convidar ${member.email} para subevento:`, err));
        }
      }
      ...
      <div className="max-w-2xl mx-auto px-6 py-12 relative z-10">
            ...
            return (
              <div key={label} className="flex items-center">
                <div className="flex flex-col items-center gap-1.5">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${done ? "bg-gradient-to-br from-[#8b5cf6] to-[#9333ea] text-white" : active ? "bg-primary/10 border-2 border-primary text-primary" : "bg-sidebar border border-border text-accent-foreground/40"}`}>
                    {done ? <Check size={14} /> : i + 1}
                  </div>
                  <span className={`text-[11px] font-bold whitespace-nowrap transition-colors ${active ? "text-primary" : done ? "text-accent-foreground/60" : "text-accent-foreground/30"}`}>
                    {label}
                  </span>
                </div>
                {i < STEP_LABELS.length - 1 && <div className={`w-12 h-px mx-1 mb-6 transition-colors ${done ? "bg-primary/40" : "bg-border"}`} />}
              </div>
            );
          })}
        </div>
        ...
        <div className="bg-[#11101B] border border-border rounded-xl p-6 md:p-8 animate-in fade-in slide-in-from-bottom-6 duration-400">
          ...
          {step === 0 && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-accent-foreground mb-1.5">Data início *</label>
                  <input
                    type="date"
                    value={form.date_start}
                    onChange={(e) => handleDateChange("date_start", e.target.value)}
                    className={`w-full px-4 py-2.5 text-accent-foreground rounded-lg text-sm bg-[#11101B] border ${errors.date_start ? "border-red-400/50" : "border-border"} focus:border-primary outline-none`}
                  />
                  {errors.date_start && <p className="text-xs text-red-400 mt-1">{errors.date_start}</p>}
                </div>
                <div>
                  <label className="block text-sm font-bold text-accent-foreground mb-1.5">Horário início *</label>
                  <input
                    type="time"
                    value={form.time_start}
                    onChange={(e) => set("time_start", e.target.value)}
                    className={`w-full px-4 py-2.5 text-accent-foreground rounded-lg text-sm bg-[#11101B] border ${errors.time_start ? "border-red-400/50" : "border-border"} focus:border-primary outline-none`}
                  />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-accent-foreground mb-1.5">Data término *</label>
                  <input
                    type="date"
                    value={form.date_end}
                    onChange={(e) => handleDateChange("date_end", e.target.value)}
                    className={`w-full px-4 py-2.5 text-accent-foreground rounded-lg text-sm bg-[#11101B] border ${errors.date_end ? "border-red-400/50" : "border-border"} focus:border-primary outline-none`}
                  />
                  {errors.date_end && <p className="text-xs text-red-400 mt-1">{errors.date_end}</p>}
                </div>
                <div>
                  <label className="block text-sm font-bold text-accent-foreground mb-1.5">Horário término *</label>
                  <input
                    type="time"
                    value={form.time_end}
                    onChange={(e) => set("time_end", e.target.value)}
                    className={`w-full px-4 py-2.5 text-accent-foreground rounded-lg text-sm bg-[#11101B] border ${errors.time_end ? "border-red-400/50" : "border-border"} focus:border-primary outline-none`}
                  />
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`flex flex-col items-start gap-1 px-4 py-3 rounded-lg border text-left transition-all text-sm font-bold cursor-pointer
                      ${form.isPublic === true ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "border-white/[0.06] text-accent-foreground/60 hover:border-purple-500/30"}`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-col items-start gap-1 px-4 py-3 rounded-lg border text-left transition-all text-sm font-bold cursor-pointer
                      ${form.isPublic === false ? "bg-amber-500/10 border-amber-500/20 text-amber-400" : "border-white/[0.06] text-accent-foreground/60 hover:border-purple-500/30"}`}
                  >
                ...
                <p className="text-xs text-accent-foreground/40 mt-1">
                  {form.isPublic ? "Qualquer pessoa poderá ver e se inscrever." : "Apenas pessoas com link ou convidadas poderão acessar."}
                </p>
              </div>
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-bold text-accent-foreground mb-1.5">Membros da Equipe</label>
                <p className="text-xs text-accent-foreground/40 mb-3">
                  Adicione os membros da equipe organizadora. Eles receberão um convite por e-mail.
                </p>
          ...
          {step === 2 && (
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-bold text-accent-foreground mb-1.5">
                  Subeventos / Atividades <span className="text-primary">*</span>
                </label>
                <p className="text-xs text-accent-foreground/40 mb-3">
                  Adicione as atividades que compõem seu evento. Cada subevento pode ter múltiplas seções (horários).
                </p>
                {hasEventDates && (
                ...
                {errors.subevents && <p className="text-xs text-red-400 mb-3">{errors.subevents}</p>}

                {/* Lista de subeventos existentes */}
                {subevents.length > 0 && (
                  <div className="space-y-3 mb-4">
                    {subevents.map((sub, idx) => (
                      <div key={idx} className="p-3 rounded-lg bg-[#11101B] border border-border">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="font-bold text-accent-foreground">{sub.name}</p>
                            <div className="flex items-center gap-3 text-xs text-accent-foreground/60 mt-1">
                              <span className="flex items-center gap-1"><MapPin size={12} /> {sub.location}</span>
                            </div>
                            <div className="flex items-center gap-3 text-xs text-accent-foreground/60 mt-1">
                              <span className="flex items-center gap-1"><CalendarDays size={12} /> {sub.sections.length} seção(ões)</span>
                              <span className="flex items-center gap-1"><Users size={12} /> {sub.teamMembers.length} membro(s)</span>
                            </div>
                          </div>
                          <div className="flex gap-1">
                            <button onClick={() => editSubevent(idx)} className="p-1.5 rounded-md bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 cursor-pointer">
                              <Edit2 size={14} />
                            </button>
                            <button onClick={() => removeSubevent(idx)} className="p-1.5 rounded-md bg-red-500/10 text-red-400 hover:bg-red-500/20 cursor-pointer">
                              <Trash2 size={14} />
                            </button>
                          </div>
                        </div>
                ...
                <div className="border-t border-border pt-4">
                  <p className="text-sm font-semibold text-accent-foreground mb-3">
                    {activeSubeventIndex !== null ? "Editando subevento" : "Novo subevento"}
                  </p>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="border-t border-border/50 pt-3">
                      ...
                      {newSubevent.sections.length > 0 && (
                        <div className="space-y-2 mb-3">
                          {newSubevent.sections.map((section) => (
                            <div key={section.id} className="flex justify-between items-center p-2 rounded-lg bg-[#11101B] border border-border/50">
                              <div className="flex flex-col">
                                ...
                                <span className="text-xs text-emerald-400 mt-1">Capacidade: {section.capacity}</span>
                              </div>
                              <button onClick={() => removeSectionFromSubevent(section.id)} className="text-red-400 cursor-pointer">
                                <Trash2 size={12} />
                              </button>
                            </div>
                          ))}
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-2">
                        ...
                        <div className="col-span-2">
                          <label className="block text-xs font-semibold text-accent-foreground mb-1">Capacidade da seção *</label>
                          <input
                            type="number"
                            value={newSection.capacity}
                            onChange={(e) => setNewSection((prev) => ({ ...prev, capacity: e.target.value }))}
                            placeholder="ex: 30"
                            className={`w-full px-3 py-1.5 text-accent-foreground rounded-lg text-xs bg-[#11101B] border ${errors.sectionCapacity ? "border-red-400/50" : "border-border"} focus:border-primary outline-none`}
                          />
                      ...
                      {!hasEventDates && <p className="text-xs text-red-400 mt-1">Preencha as datas e horários do evento principal antes de adicionar seções.</p>}
                    ...
                    <div className="border-t border-border/50 pt-3">
                      ...
                      {newSubevent.teamMembers.length > 0 && (
                        <div className="space-y-1 mb-2">
                          {newSubevent.teamMembers.map((member) => (
                            <div key={member.id} className="flex justify-between items-center p-1.5 rounded-lg bg-[#11101B] border border-border/50">
                              <span className="text-xs text-accent-foreground/60">{member.email} - {member.job}</span>
                              <button onClick={() => removeMemberFromSubevent(member.id)} className="text-red-400 cursor-pointer">
                                <Trash2 size={10} />
                              </button>
                            </div>
                          ))}
                      ...
                      {teamMembers.length > 0 && (
                        <div className="mb-2">
                          <select onChange={(e) => selectExistingMember(e.target.value)} value="" className="w-full px-3 py-1.5 text-accent-foreground rounded-lg text-xs bg-[#11101B] border border-border focus:border-primary outline-none">
                            <option value="">-- Selecionar da equipe principal --</option>
                            {teamMembers.map((m) => (
                              <option key={m.id} value={m.email}>{m.email} ({m.job})</option>
                            ))}
                          </select>
                        </div>
                      )}
                      ...
                      <div className="flex gap-2 mb-2">
                        <input
                          ...
                          onChange={(e) => setNewSubeventMember((prev) => ({ ...prev, email: e.target.value, fromExisting: false }))}
                          placeholder="E-mail"
                      ...
                      {(errors.subeventMemberEmail || errors.subeventMemberJob) && <p className="text-xs text-red-400 mt-1">{errors.subeventMemberEmail || errors.subeventMemberJob}</p>}
                    </div>

                    <button onClick={saveSubevent} className="w-full flex cursor-pointer items-center justify-center gap-2 py-2 rounded-lg text-sm font-bold text-white bg-gradient-to-br from-purple-600 to-purple-700 hover:opacity-90 transition-all">
                      {activeSubeventIndex !== null ? "Atualizar subevento" : "Adicionar subevento"}
                    </button>
                  </div>
                </div>
              </div>
          ...
          {step === 3 && (
            <div className="space-y-6">
              <p className="text-sm text-accent-foreground/60">Confira as informações antes de publicar.</p>

              <div>
                <div className="text-[11px] font-bold uppercase text-accent-foreground/40 mb-3">Evento Principal</div>
                <div className="space-y-2">
                  <div className="flex gap-3 p-3 rounded-lg bg-[#11101B] border border-border">
                    ...
                    <div><div className="text-[10px] uppercase text-accent-foreground/40">Nome</div><div className="text-sm text-accent-foreground">{form.name}</div></div>
                  </div>
                  <div className="flex gap-3 p-3 rounded-lg bg-[#11101B] border border-border">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-accent-foreground">
                        {formatDate(form.date_start, form.time_start)} {form.time_start ? `às ${form.time_start}` : ""} → {formatDate(form.date_end, form.time_end)} {form.time_end ? `às ${form.time_end}` : ""}
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-3 p-3 rounded-lg bg-[#11101B] border border-border">
                    <MapPin size={18} className="text-accent-foreground/40" />
                    <div><div className="text-[10px] uppercase text-accent-foreground/40">Local</div><div className="text-sm text-accent-foreground">{form.location}</div></div>
                  </div>
                  <div className="flex gap-3 p-3 rounded-lg bg-[#11101B] border border-border">
                    {form.isPublic ? <Globe size={18} className="text-emerald-400" /> : <Lock size={18} className="text-red-400" />}
                    <div>
                      <div className="text-[10px] uppercase text-accent-foreground/40">Visibilidade</div>
                      <div className="text-sm text-accent-foreground">{form.isPublic ? "Público" : "Privado"}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {teamMembers.length > 0 && (
                <div>
                  <div className="text-[11px] font-bold uppercase text-accent-foreground/40 mb-3">Equipe do Evento ({teamMembers.length})</div>
                  <div className="space-y-1">
                    {teamMembers.map((m) => (
                      <div key={m.id} className="text-xs text-accent-foreground/60">{m.email} - {m.job}</div>
                    ))}
                  </div>
                </div>
              )}
              ...
              {subevents.length > 0 && (
                <div>
                  <div className="text-[11px] font-bold uppercase text-accent-foreground/40 mb-3">Subeventos ({subevents.length})</div>
                  <div className="space-y-2">
                    {subevents.map((sub, idx) => (
                      <div key={idx} className="p-3 rounded-lg bg-[#11101B] border border-border">
                        <div className="font-bold text-accent-foreground">{sub.name}</div>
                        <div className="flex items-center gap-3 text-xs text-accent-foreground/60 mt-1">
                          <span className="flex items-center gap-1"><MapPin size={12} /> {sub.location}</span>
                        </div>
                        <div className="mt-2">
                          <div className="text-[10px] uppercase text-accent-foreground/40">Seções</div>
                          {sub.sections.map((sec, sIdx) => (
                            <div key={sIdx} className="text-xs text-accent-foreground/60 mt-1 ml-2">
                              • {sec.title ? `${sec.title} - ` : ""}{formatDate(sec.date_start, sec.time_start)} {sec.time_start ? `às ${sec.time_start}` : ""} → {formatDate(sec.date_end, sec.time_end)} {sec.time_end ? `às ${sec.time_end}` : ""}
                              {sec.capacity && <span className="ml-2 text-emerald-400">(Cap: {sec.capacity})</span>}
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              {status === "error" && (
                <div className="flex gap-3 p-3 rounded-lg bg-red-400/5 border border-red-400/20">
                  ...
                  <div><div className="text-sm font-bold text-red-400">Falha ao criar evento</div><div className="text-xs text-red-400/70">{errMsg}</div></div>
                </div>
              )}
            </div>
          )}
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-border">
            <button onClick={handleBack} className="flex cursor-pointer items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium text-accent-foreground/60 border border-border hover:text-accent-foreground hover:border-primary/30 transition-all">
            ...
            {step < STEP_LABELS.length - 1 ? (
              <button onClick={next} className="flex cursor-pointer items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold text-white bg-gradient-to-br from-[#8b5cf6] to-[#9333ea] shadow-md hover:shadow-lg transition-all">
                Próximo <ArrowRight size={14} />
              </button>
            ) : (
              <button onClick={handleSubmit} disabled={creatingEvent} className="flex cursor-pointer items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold text-white bg-gradient-to-br from-[#8b5cf6] to-[#9333ea] shadow-md hover:shadow-lg transition-all disabled:opacity-50">
                {creatingEvent ? <><div className="w-3.5 h-3.5 rounded-full border-2 border-white/30 border-t-white animate-spin" /> Criando...</> : <><Sparkles size={14} /> Criar Evento</>}
              </button>
            )}
          </div>
        </div>
        <p className="text-center text-xs text-accent-foreground/30 mt-6">Certificados gerados automaticamente após o evento.</p>
      </div>
    </div>
  );
}